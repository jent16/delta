...
const RESUME_SKILLS_TOOL = {
  ...
  description:
    "Record the technical skills a candidate demonstrates in their resume, and the industry their experience is best suited for.",
  input_schema: {
    type: "object",
    properties: {
      skills: {
      ...
      inferred_industry: {
        type: "string",
        description:
          "Best single-word-or-short-phrase guess at the industry/sector this candidate's experience is most aligned with (e.g. Technology, Fintech, Healthcare), based on their resume content.",
      },
    },
    required: ["skills", "inferred_industry"],
  },
};

// returns { skills, inferredIndustry }
    ...
    messages: [
      {
        ...
        content:
          ...
          `Also give your best-guess industry for this candidate.`,
      },
    ],
  });
  const toolUse = msg.content.find((b) => b.type === "tool_use");
  if (!toolUse) return { skills: [], inferredIndustry: null };
  return { skills: toolUse.input.skills, inferredIndustry: toolUse.input.inferred_industry || null };
}
