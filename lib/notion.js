...
const FIELD_NAMES = {
  ...
  posted: ["posted", "date posted", "posting date", "posted date", "listed"],
...
function buildProperties(schemaProps, listing) {
  ...
  const titleProp = Object.entries(schemaProps).find(([, p]) => p.type === "title");
  if (!titleProp) throw new Error("that Notion database has no title property");

  const properties = {
    [titleProp[0]]: { title: text(`${listing.title}${listing.company ? ` — ${listing.company}` : ""}`) },
  };
  const filled = [titleProp[0]];
  const values = fieldValues(listing);

  for (const [field, names] of Object.entries(FIELD_NAMES)) {
    const match = names.map((n) => byLower.get(n)).find(Boolean);
