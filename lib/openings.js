...
// Looks a listing up by Simplify's stable id across both datasets, so the
// server never has to trust listing details sent back from the browser.
async function findListingById(id) {
  for (const type of ["intern", "newgrad"]) {
    const hit = (await fetchListings(type)).find((d) => d.id === id);
    if (hit) return shapeListing(hit);
  }
  return null;
}
...
async function findOpenings(opts) {
  ...
  const listings = filtered.slice(0, limit).map(shapeListing);
...
module.exports = { findOpenings, findListingById, CATEGORY_GROUPS, parseLocation, degreeMatches, parseTerm, termWindow, isTermRelevant };
