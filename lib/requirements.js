...
function loadPostings(db, roleId) {
  const postings = db
    .prepare(
      `SELECT posting_id, company, title, url, track, fetched_at
...
function roleRequirements(db, roleId) {
  ...
  return { postings: postings.length, minimum, preferred, tracks, listings: postings };
}
...
function scoreRole(db, role, haveIds) {
  ...
  const fitScore = listings.length
    ? listings.reduce((sum, l) => sum + l.requiredShare, 0) / listings.length
    : 0;
