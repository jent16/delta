...
// For roles in lib/qualifications.js: score a resume against a fixed
// checklist instead of posting-derived skills. `assessment` is the cached
// [{name, evidenced, evidence}] from resumes.qualifications (computed once
// at upload time — this function itself is still pure SQL/JS, no Claude).
// Real postings/industries still come through for context and browsing,
// same traceability as skill-based roles; there just aren't per-posting
// skill requirements to show, since none were ever extracted.
function scoreQualificationRole(db, role, qualifications, assessment) {
  const req = roleRequirements(db, role.role_id);
  const byName = new Map((assessment || []).map((a) => [a.name, a]));
  const items = qualifications.map((name) => {
    const a = byName.get(name);
    return { name, evidenced: !!(a && a.evidenced), evidence: a ? a.evidence || "" : "" };
  });
  const have = items.filter((i) => i.evidenced).length;
  const fitScore = items.length ? Math.round((have / items.length) * 100) : 0;
  ...
  return {
    ...
    minimum: { have, total: items.length, missing: items.filter((i) => !i.evidenced).map((i) => i.name) },
    preferred: { have: 0, total: 0, missing: [] },
    tracks: [],
    industries: req.industries,
    listings: req.listings.map((p) => ({
      postingId: p.posting_id,
      company: p.company,
      title: p.title,
      url: p.url,
      track: p.track,
      industry: p.industry,
      required: { have: 0, total: 0, missing: [] },
      preferred: { have: 0, total: 0, missing: [] },
      requiredShare: 0,
    })),
    fitScore,
...
module.exports = { MIN_SHARE, PREF_SHARE, roleRequirements, scoreRole, scoreQualificationRole };
